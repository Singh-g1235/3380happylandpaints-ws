...
exports.getProduct = async (req, res) => {
  try {
    ...
    res.header("Content-Type", "application/json");
    res.send({
      message: `Error in finding : ${req.params.productId} . ${err.stack}`,
    });
  }
};
  ...
  try {
    var product = {
      ...
      ProductDescription: {
        ...
        sheen: req.body.sheen,
        cleanup: req.body.cleanup,
        resinType: req.body.resinType,
        recommendedUse: req.body.recommendedUse,
        mpiRating: req.body.mpiRating,
        vocLevel: req.body.vocLevel,
      },
    ...
    res.status(500);
    res.send({
      message: `Error in Adding Product with product id ${req.body.ProductId}`,
    });
  }
};
...
exports.deleteProduct = async (req, res) => {
  try {
    deletedProductResult = await Product.findOneAndDelete({
      ProductId: req.body.ProductId,
    });
...
exports.getOrder = async (req, res) => {
  try {
    ...
    res.header("Content-Type", "application/json");
    res.send({
      message: `Error in finding : ${req.params.orderId} . ${err.stack}`,
    });
  }
};
exports.shipOrder = async (req, res) => {
  try {
    ...
    updatedOrderResult = await Order.findOneAndUpdate(
    ...
    res.status(500);
    res.send({
      message: `Error in Updating Order with Order id ${req.body.OrderId}`,
    });
  }
};
...
exports.getUser = async (req, res) => {
  try {
    ...
    res.header("Content-Type", "application/json");
    res.send({
      message: `Error in finding : ${req.params.userId} . ${err.stack}`,
    });
  }
};
...
exports.updateUser = async (req, res) => {
  try {
    ...
    updatedUserResult = await SignUp.findOneAndUpdate(
      { UserId: req.body.UserId },
      user,
      options
    );
    updatedLoginResult = await Login.findOneAndUpdate(
      { UserId: req.body.UserId },
      login,
      options
    );
    ...
    res.status(500);
    res.send({
      message: `Error in Updating User with user id ${req.body.UserId}`,
    });
  }
};
exports.deleteUser = async (req, res) => {
  try {
    deletedUserResult = await SignUp.findOneAndDelete({
      UserId: req.body.UserId,
    });
    deletedLoginResult = await Login.findOneAndDelete({
      UserId: req.body.UserId,
    });
    ...
    res.status(500);
    res.send({
      message: `Error in Deleting User with user id ${req.body.UserId}`,
    });
  }
};
...
exports.approveSupply = async (req, res) => {
  try {
    var fetchProduct = await Product.findOne({
      ProductId: req.body.ProductId,
    }).exec();
    ...
    if (!fetchProduct) {
      ...
      let options = { upsert: true, new: true, setDefaultsOnInsert: true };
      var update = { $inc: { ProductQuantity: req.body.ProductQuantity } };
